...
export const handler: Schema['requestQuote']['functionHandler'] = async (
  event,
  _context
) => {
  const { email, content, name } = event.arguments;
  ...
  try {
    ...
    const templateValues = {
      ...
      LoginButtonUrl: `https://request-quote.alfajoresny.com`,
    ...
    console.log('sendEmailCommand done');
    return { success: true };
  } catch (error) {
